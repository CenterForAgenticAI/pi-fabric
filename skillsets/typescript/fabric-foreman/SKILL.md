...
```ts
const cadence = π.cadence;
if (cadence !== "turn_end" && cadence !== "agent_settled") throw new Error("Choose turn_end or agent_settled");
if (π.delivery !== "off" && π.delivery !== "steer") throw new Error("Choose off or steer");
const recordOnly = π.delivery === "off";
const definition = {
  "name": "foreman",
  "inputSchema": {
    "type": "object",
...
definition.requires = recordOnly ? ["jev.evaluate"] : ["jev.evaluate", "jev.advise"];
const observer = await jev.spawn({
  input: { goal: π.goal, acceptance: JSON.parse(π.acceptance), instructions: π.instructions, cadence, recordOnly },
  observe: {
    events: ["input", "turn_end", "agent_settled"],
    include: ["assistantText", "toolResults"],
    maxChars: 4096, queueSize: 8, maxEventAgeMs: 30000,
    ...(recordOnly ? {} : { delivery: "steer" as const, triggerTurn: true, maxAdvice: 1 }),
  },
  program: { ...definition, code: `
const questions = {
...
return { id: observer.id, state: observer.state, cadence, recordOnly };
```
